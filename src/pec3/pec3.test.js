...
describe("Exercise 2", () => {
  ...
  test("should reject with error for invalid productId type", async () => {
    await expect(fetchProductStock("abc")).rejects.toThrow(
      "Invalid product id",
    );
  });

  test("should reject with error for out of range productId", async () => {
    await expect(fetchProductStock(0)).rejects.toThrow(
      "Product id is out of valid range",
    );
    await expect(fetchProductStock(5001)).rejects.toThrow(
      "Product id is out of valid range",
    );
  });
});
...
describe("Exercise 5", () => {
  ...
  test("should process all orders successfully", async () => {
    ...
    const results = await processPendingOrders(
      fetchPendingOrders,
      processOrder,
    );
  ...
  test("should throw error when one processing order fails", async () => {
    ...
    await expect(
      processPendingOrders(fetchPendingOrders, processOrder),
...
describe("Exercise 6", () => {
  ...
  test("should get order details using async/await", async () => {
    ...
    const result = await getOrderDetailsAsync(
      fetchUserId,
      fetchOrders,
      fetchOrderDetails,
    );
